...
func (c *NotificationClient) SendReminder(ctx context.Context, invoiceId int64, email string, daysBefore int) error {
	req := &notificationpb.SendNotificationRequest{
		Email:   email,
		Message: fmt.Sprintf("Reminder: Invoice %d is due in %d days", invoiceId, daysBefore),
	}
